\documentclass{ximera}

\input{../../../preamble.tex}



\outcome{Identify pairs in a relation.}


\begin{document}



 

\begin{exercise}
Below is the drawing of an interval.  What is the interval notation for this interval?
\begin{image}
\begin{tikzpicture}
	\begin{axis}[
            domain=-2:4,
            width=6in,
            height=1in,
            axis lines =middle, xlabel=$ $, ylabel=$ $,
            ymajorticks=false,
            every axis y label/.style={at=(current axis.above origin),anchor=south},
            every axis x label/.style={at=(current axis.right of origin),anchor=west},
            clip=false,
            %axis on top,
          ]
          \addplot [textColor, very thin, domain=(-5:5)] {0}; % puts the axis back, axis on top clobbers our open holes
          \addplot [textColor, very thin] plot coordinates {(0,-0.3) (0,0.3)}; % puts the axis back, axis on top clobbers our open holes

          \addplot [very thick, penColor, domain=(0:1)] {0.1};
          \addplot[color=penColor,fill=penColor,only marks,mark=*] coordinates{(0,0.1)};  %% closed hole          
          \addplot[color=penColor,fill=background,only marks,mark=*] coordinates{(1,0.1)};  %% open hole

        \end{axis}
\end{tikzpicture}
\end{image}


\begin{multipleChoice}
\choice{$[0, 1]$}
\choice{$(0, 1]$}
\choice{$[0, 1)$}
\choice{$(0, 1)]$}
\end{multipleChoice}

\end{exercise}










\begin{exercise}
Select all valid interval notations.
\begin{selectAll}
\choice[correct]{$(0, 4]$}
\choice{$(3, 0]$}
\choice{$[0, -2]$}
\choice{$(-3, \infty]$}
\end{selectAll}
\end{exercise}






\begin{exercise}
Select all invalid interval notations.
\begin{selectAll}
\choice{$(0, 6]$}
\choice{$(4, 0]$}
\choice{$[0, -3]$}
\choice[correct]{$(0, \infty)$}
\end{selectAll}
\end{exercise}





\begin{exercise}
Below is the drawing of an interval.  What is the interval notation for this interval?
\begin{image}
\begin{tikzpicture}
	\begin{axis}[
            domain=-2:4,
            width=6in,
            height=1in,
            axis lines =middle, xlabel=$ $, ylabel=$ $,
            ymajorticks=false,
            every axis y label/.style={at=(current axis.above origin),anchor=south},
            every axis x label/.style={at=(current axis.right of origin),anchor=west},
            clip=false,
            %axis on top,
          ]
          \addplot [textColor, very thin, domain=(-5:5)] {0}; % puts the axis back, axis on top clobbers our open holes
          \addplot [textColor, very thin] plot coordinates {(0,-0.3) (0,0.3)}; % puts the axis back, axis on top clobbers our open holes

          \addplot [very thick, penColor, domain=(-3:2)] {0.1};
          \addplot[color=penColor,fill=penColor,only marks,mark=*] coordinates{(-3,0.1)};  %% closed hole          
          \addplot[color=penColor,fill=penColor,only marks,mark=*] coordinates{(2,0.1)};  %% open hole

        \end{axis}
\end{tikzpicture}
\end{image}


\begin{multipleChoice}
\choice{$[2, -3]$}
\choice{$(2, -3]$}
\choice{$[-3, 2)$}
\choice[correct]{$[-3, 2)]$}
\end{multipleChoice}

\end{exercise}






\begin{exercise}
Select all valid interval notations.
\begin{selectAll}
\choice{$(0, -\infty]$}
\choice{$(3, -\infty)$}
\choice[correct]{$[0, 5]$}
\choice[correct]{$(-\infty, -3]$}
\end{selectAll}
\end{exercise}






\begin{exercise}
Select all invalid interval notations.
\begin{selectAll}
\choice[correct]{$(3, -3]$}
\choice{$(-4, 0]$}
\choice[correct]{$[-\infty, -3]$}
\choice[correct]{$[-\infty, \infty]$}
\end{selectAll}
\end{exercise}









\end{document}
