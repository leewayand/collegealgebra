...
\item $DoubleVowel(string)$ = number of consonants in $string + 2Ÿ \cdot$ number of vowels in $string$.
\end{itemize}
...
\item[\textbf{ANSWER}] No. If the string has $m$ consonants with $k$ vowels, then $DoubleVowel$ has the value $m + 2Ÿ\cdot k$. 
For any natural number, $N$, we can values for $m$ and $k$, such that $m + 2Ÿ\cdot k = N$.
\end{dialogue}
